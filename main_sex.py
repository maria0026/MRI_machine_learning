...
def main(args):
    ...
    for i in range(args.n_crosval):
        ...
        X_train, X_test, y_train, y_test=prepare_dataset.split_dataset(df, args.label_names, args.test_size)
        ...
        #PCA
        pca_mri, train_pca, test_pca, importance_df=dimensions_reduction.principal_component_analysis(X_train, X_test, args.components_nr, n_features=args.n_most_important_features)
        ...
        if args.model_name=="forest":
            ...
            accuracy, precision, recall, FPR, cm, fpr, tpr=test.recurrent_neural_network_classification(X_test, y_test, args.rnn_seq_dim, input_dim, model)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--_fnn_weight_decay", nargs="?", default=0.08, help="Weight decay for feed forward neural network", type=float)
