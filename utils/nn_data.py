...
class Data(Dataset):
    ...
    def __getitem__(self, index):
        return self.X[index], self.y[index], self.identifiers[index]
   
    def __len__(self):
        return self.len
    
class DataRNN(Dataset):
    def __init__(self, X, y, identifiers, sequence_length):
...
def load_fnn_data(X, y, batch_size, feature):
    train_data = Data(X, y[feature], y[feature].values)
    train_dataloader = DataLoader(dataset=train_data, batch_size=batch_size, shuffle=True)

    return train_dataloader

def load_rnn_data(X, y, batch_size, feature):
    train_data = DataRNN(X, y[feature], y[feature].values, sequence_length=1)
    train_dataloader = DataLoader(dataset=train_data, batch_size=batch_size, shuffle=True)
