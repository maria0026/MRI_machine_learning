...
def main(args):
    ...
    all_concatenated=pd.DataFrame()

    for i, filepath in enumerate(args.path):
        processor = prepare_csv.FileProcessor(filepath)
        ...
        columns=['Brain_Segmentation_Volume','Brain_Segmentation_Volume_Without_Ventricles','Brain_Segmentation_Volume_Without_Ventricles_from_Surf','Total_cortical_gray_matter_volume','Supratentorial_volume','Supratentorial_volume.1', 'Estimated_Total_Intracranial_Volume','Brain Segmentation Volume',	'Brain Segmentation Volume Without Ventricles', 'White_Surface_Total_Area', 'Mean_Thickness', 'Left hemisphere cortical gray matter volume', 'Right hemisphere cortical gray matter volume','ASEG-Left hemisphere cerebral white matter volume','ASEG-Right hemisphere cerebral white matter volume', 'ASEG-Total cerebral white matter volume','ASEG-Subcortical gray matter volume', 'ASEG-Total gray matter volume', 'ASEG-Supratentorial volume', 'ASEG-Mask Volume', 'ASEG-Supratentorial volume_notvent', 'Supratentorial volume.1', 'Total_cerebral_white_matter_volume', 'Volume of ventricles and choroid plexus', 'Left_hemisphere_cerebral_white_matter_volume', 'Right_hemisphere_cerebral_white_matter_volume']
        filenames=['LHA2009.csv', 'LHAPARC.csv', 'LHDKT.csv', 'RHA2009.csv', 'RHAPARC.csv', 'RHDKT.csv', 'ASEG.csv', 'BRAIN.csv', 'WM.csv']
        ...
        for filename in filenames:
            ...
            if os.path.isfile(filename):
                print("rozmiar", pd.read_csv(f'{filepath}/{filename}', sep='\t').shape)
                if len(indexes_df)>0:
        ...
        atlas=os.path.split(filepath)[-1]
        folder_out=f'data/preprocessed_atlas/{args.data_type}_norm_confirmed_{atlas}'
        if i==0:
            folder_out_all=folder_out
        if i>0:
            folder_out_all=folder_out_all+f'_{atlas}'
        ...
        if all_concatenated.empty:
            all_concatenated = concatenated_df
        else:
            common_columns = set(all_concatenated.columns).intersection(set(concatenated_df.columns)) - {'identifier'}
            concatenated_df = concatenated_df.drop(columns=common_columns)
            
            all_concatenated = all_concatenated.merge(concatenated_df, on='identifier', how='outer')


    print("all concatenated_df", all_concatenated.shape, "to", f'{folder_out_all}/all_concatenated.csv')
    if not os.path.exists(folder_out_all):
        os.makedirs(folder_out_all)
    all_concatenated.to_csv(f'{folder_out_all}/all_concatenated.csv', sep='\t', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", nargs="?", default=['data/original_atlas/a2009', 'data/original_atlas/ASEG'], help="Path to the folder where the original files are", type=list)
