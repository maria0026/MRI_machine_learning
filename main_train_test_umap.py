...
def main(args):
    ...
    for i in range(global_config['n_crosval']):
        ...
        #umap
        reducer, train_umap, val_umap, test_umap = reductor.umap(X_train, X_test, X_val=X_val, validation=args.valid, n_components=args.components_nr)
        joblib.dump(reducer, f'{model_path}_umap_mri_train_nr_{i}.pkl')

        train_principal_Df = pd.DataFrame(data = train_umap
                    , columns = [str(i) for i in range(1,train_umap.shape[1]+1)], index=X_train.index)
        
        if args.valid:
            val_principal_Df = pd.DataFrame(data = val_umap
                        , columns = [str(i) for i in range(1,val_umap.shape[1]+1)], index=X_val.index)

        test_principal_Df = pd.DataFrame(data = test_umap
                    , columns = [str(i) for i in range(1,test_umap.shape[1]+1)], index=X_test.index)
        ...
        if args.model_name=='forest':
            ...
            joblib.dump(model, f'{model_path}/model_train_nr_{i}.pkl')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--components_nr", nargs="?", default=40, help="Number of components for principal component analysis", type=int)
